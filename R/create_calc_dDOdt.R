...
create_calc_dDOdt <- function(data, ode_method, GPP_fun, ER_fun, deficit_src, err.proc=0) {
  ...
  D <- switch(
    ...
    DO_obs=(function(){
      ...
      if(is.null(DO.obs)){ # if two-station data
        if(integer.t) function(t, metab.pars, DO.mod.t) {
          metab.pars[['K600.daily']] * KO2.conv[t] * tt[t] * ((DO.sat.up[t] + DO.sat.down[t+lag])/2 - (DO.obs.up[t] + DO.obs[t+lag])/2)
    ...
    DO_mod=(function(){
      ...
      if(is.null(DO.obs)) { # if two-station data
        if(integer.t) function(t, metab.pars, DO.mod.t) {
          metab.pars[['K600.daily']] * KO2.conv[t] * tt[t] * ((DO.sat.up[t] + DO.sat.down[t+lag])/2 - (DO.obs.up[t] + DO.mod.t[t+lag])/2)
        } else function(t, metab.pars, DO.mod.t) {
          metab.pars[['K600.daily']] * KO2.conv(t) * tt(t) * ((DO.sat.up(t) + DO.sat.down(t+lag))/2 - (DO.obs.up(t) + DO.mod.t(t+lag))/2)
        }
